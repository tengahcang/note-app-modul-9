  ...
  return (
    <Box flex={1} backgroundColor="$white" justifyContent="center">
      ...
      <Box shadowColor="$black" shadowOffset={{ width: 0, height: 2 }} shadowOpacity={"$25"} shadowRadius={"$3.5"} elevation={"$5"} backgroundColor="$white" borderRadius={"$md"} mt={"$8"} mx={"$3"} px={"$3"} pt={"$2"}>
        <Heading size="2xl" color="$black"> Edit Your Task! </Heading>
        <Text size="sm" color="$black" my={"$1"}> Having a mistake? An edit got you covered! </Text>
        <FormControl>
          ...
          <Button type="text" title="Update" padding={10} onPress={() => { onEditNote(); }} />
        </FormControl>
      </Box>
      {/* show Alert */}
      {showAlert && (
